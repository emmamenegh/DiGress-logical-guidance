""" 
Supports guidance towards the satisfaction of the Lipinski rules with a fully-shared classifier architecture.
"""
import os
import sys
...
from src.guidance.guidance_diffusion_model_discrete_full_shared import DiscreteDenoisingDiffusion # same
from src.datasets import guacamol_dataset_full_shared as guacamol_dataset # same
...
from src.guidance.guacamol_classifier_discrete_full_shared import GuacamolClassifierDiscrete
...
def main(cfg: DictConfig):
    ...
    if cfg.general.trained_classifier_dir is not None and cfg.general.guidance_target is not None:
        model_path = os.path.join(cfg.general.trained_classifier_dir, 'full_shared.ckpt')
        ...
        print(f"--- Loaded config for classifier: ---")
        for k, v in loaded_cfg.items():
            print(f"  {k}: {v}")
        print("--- End loaded config ---")

        guidance_models[0] = GuacamolClassifierDiscrete.load_from_checkpoint(model_path,
                                                                            train_metrics=train_metrics,
                                                                            sampling_metrics=sampling_metrics)
        guidance_models[0].to(device)
    else:
