...
from src.datasets.guacamol_dataset import LipinskiBinaryExtractor
...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    ...
    def cond_fn(self, noisy_data, node_mask):
        ...
        with torch.enable_grad():
            ...
            # Predict properties with guidance models
            logits = []
            for idx, model in self.guidance_models.items():
                model = model.to(device)
                pred = model.model(x_in, e_in, y, node_mask)
                logits.append(pred.y.view(-1))
            logits = torch.stack(logits, dim=1)

            if self.guidance_target == "all" and self.satisfaction_rule == "bce_baseline":
                target = torch.ones_like(logits) # (b, num_properties)
                
                bce_per_label = F.binary_cross_entropy_with_logits(logits, target, reduction='none') # (b, num_properties)
                bce = bce_per_label.sum(dim=1) # (b,)
            
            else:
                if self.guidance_target in ["logP", "molW", "HBD", "HBA"] and self.satisfaction_rule == "single":
                    satisfaction_prob = single_property_satisfaction(logits)
                elif self.guidance_target == "all" and self.satisfaction_rule == "composite":
                    satisfaction_prob = composite_property_satisfaction(logits)
                elif self.guidance_target == "all" and self.satisfaction_rule == "LRo5":
                    satisfaction_prob = LRo5_satisfaction(logits)
                else:
                    raise ValueError(f"Unsupported combination: guidance_target={self.guidance_target}, satisfaction_rule={self.satisfaction_rule}")
