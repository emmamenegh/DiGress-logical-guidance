...
class ReshapeYTransform:
    def __call__(self, data):
        if data.y.ndim == 1 and data.y.numel() == 4:
            data.y = data.y.unsqueeze(0)  # shape (1, 4)
        return data
...
class GuacamolDataModule(MolecularDataModule):
    ...
    def prepare_data(self) -> None:
        ...
        elif self.classifier and target == "all":
            transform = ReshapeYTransform()
